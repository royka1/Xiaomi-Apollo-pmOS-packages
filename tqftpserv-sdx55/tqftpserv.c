...
/*
 * QRTR sockets have a modest send buffer. With the SDX55's negotiated
 * blksize=7680 and wsize=10 a full window is ~75 kB, and send() starts
 * failing partway through the window (observed: blocks 1-7 fine, block 8
 * fails with -1). The lost blocks stall the transfer and the modem
 * ERRFATALs about a minute later. Retry on the transient errors instead of
 * dropping the block on the floor.
 */
static ssize_t send_retry(int sock, const void *buf, size_t len)
{
			      ...
			      unsigned int block, size_t offset, size_t response_size)
{
	...
	*p++ = block & 0xff;
	...
	return send_retry(client->sock, buf, send_len);
}
...
int main(int argc, char **argv)
{
	...
	/*
	 * The external SDX55 on Apollo looks up service 4096 *instance 3*
	 * (see its own F3 log: "name = [sid = 4096, iid = 3]"), not the
	 * instance 1 an on-die MPSS uses. qrtr_publish() packs the instance
	 * field as version | (instance << 8), so pass 3 as the version.
	 */
	ret = qrtr_publish(fd, 4096, 3, 0);
	if (ret < 0) {
